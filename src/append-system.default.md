...
`AGENTS.md` (agent registry, tool policies, concurrency, error recovery) and `PROTO.md` (phase workflow, commit conventions) are in your project context and are authoritative. Your role-specific protocol is at `skill://elon` — read it if you need the complete orchestration procedure. The per-agent tool policies are now **enforced** by each agent's definition plus this session's `enforce-orchestrator` extension; they are no longer advisory.
