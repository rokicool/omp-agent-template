...
- **`elon_ko.sh` now deploys project-context steering files to the current directory.** On top of installing the plugins, every install (GLOBAL and LOCAL) writes **`<cwd>/AGENTS.md`** and **`<cwd>/PROTO.md`** into the project you run it from — in the project directory itself, not the omp home. `AGENTS.md` is the file omp auto-loads (walk-up from cwd), so the team's agent registry/enforcement protocol becomes load-bearing context for that project; the installer overwrites it on every install and fails hard if it cannot fetch it (it is the one load-bearing scaffold file). `PROTO.md` is a doc-only reference (never auto-loaded); overwrite-always, non-fatal on fetch failure. Both are fetched from the same ref as Plugin A (`OMP_AGENT_REF`), so the deployed guidance matches the installed gate. `APPEND_SYSTEM.md` is **not** copied — it is already load-bearing via Plugin A's bundled default; override it per project with `<cwd>/.omp/APPEND_SYSTEM.md`. `RULES.md` is no longer a standalone scaffold file: its content ships as the always-apply `rules/ro-orchestrator-invariant.md` rule inside Plugin A (effective wherever Plugin A is installed). Uninstall leaves `<cwd>/AGENTS.md`/`<cwd>/PROTO.md` in place and prints a notice. *Release-timing note:* the rule move and the `AGENTS.md` coherence edit are in the current working tree; the stable `#v2.3.1` default fetches Plugin A and the scaffold files from the published `v2.3.1` tag, which predates them — they reach stable consumers once the next tag is cut and `OMP_AGENT_REF` is bumped; pre-release/explicit-tag installs get them now.
