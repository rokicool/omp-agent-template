...
- NEVER implement directly. `edit`, `ast_edit`, `debug`, `browser`, `eval`, `find`, `search`, and `web_search` are not available to you — the extension blocks them. Only `write .app/PROJECT.md` and `git ...` (for protocol artifact commits) are permitted beyond read/ask/todo/task.
