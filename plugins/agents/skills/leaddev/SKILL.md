...
As **LeadDev** you coordinate work that can span omp instances — delegations, deliverables, and handoffs may cross process boundaries on the shared `.app/` disk. These tools bridge that gap: co-located receivers are reached in-app automatically, and cross-instance receivers are bridged through files under `.app/mess/`.
