...
if [ -n "$TAG" ]; then
  ...
  MODE="stable"
  REF="${OMP_AGENT_REF:-v2.1.2}"        # static tag: avoids store ref-drift + network deps; OMP_AGENT_REF overrides for dev
fi
...
# (its name depends on the ref — e.g. v2.1.2 → elon-ko-2.1.2).
