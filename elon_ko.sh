...
if [ "$INSTALL_MODE" = local ]; then
  ...
  # Everything below lands under $OMP_LOCAL_HOME (PI_CONFIG_DIR + BUN_INSTALL +
  # PI_INSTALL_DIR exported above). Nothing under $HOME is touched (§6.3 guard).
  ...
  # L1: create the local tree. (plugins/ is created by omp itself under the
  # relocated config root — do NOT pre-create it; its existence is the §5.3
  # post-install relocation proof.)
  mkdir -p "$OMP_LOCAL_HOME/bin" "$OMP_LOCAL_HOME/prerelease"
...
Re-run from the project root, or report this if the project lives outside \$HOME."
  ...
  # L7: marketplace registration (commands unchanged; land in $OMP_LOCAL_HOME
  # because PI_CONFIG_DIR is exported). R6: no --scope project / link.
  ...
  # L8: Plugin A (commands unchanged) → $OMP_LOCAL_HOME/plugins/.
  ...
  # L9: Plugin B (commands unchanged; --force = idempotent) → $OMP_LOCAL_HOME/plugins/.
